...
Artist::Artist(QString connectionName, QObject *parent) : QObject(parent)
{
    this->_connectionName = connectionName;
}

int Artist::insert(const QVariantList &data) const
{
    int id = checkId(data);
    if (id != -1)
        return id;
...
void Artist::remove(const int id)
{
    QSqlQuery query(QSqlDatabase::database(_connectionName));
...
void Artist::update(const int id, const QVariantList &data)
{

}

int Artist::checkId(const QVariantList &data) const
{
    int id = -1;
